    ...
    <Box maxW={'960px'} m={'auto'} p={2}>
      ...
      <Box id="discount_codes" m={'50px 0 10px'}>
        ...
        <Modal isOpen={isOpen} onClose={() => { setIsOpen(false) }}>
          <ModalOverlay />
          <ModalContent>
            <ModalHeader>{selectedDiscount?.storename} discount code</ModalHeader>
            <ModalCloseButton />
            <ModalBody textAlign={'center'}>
              {(selectedDiscount?.type === 'free') ? (
                <Text fontWeight={600} p={5}>
                  Free {selectedDiscount?.storename} discount code
                </Text>
              ) : (
                <Text fontWeight={600} p={5}>
                  Save {selectedDiscount?.price_new}% on your next order
                </Text>
              )}
              <Text p={2}>Don't forget to apply the code at checkout</Text>
